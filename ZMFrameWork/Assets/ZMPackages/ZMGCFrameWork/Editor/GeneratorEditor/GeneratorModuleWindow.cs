...
public class GeneratorModuleWindow : EditorWindow
{
    ...
    public void OnGUI()
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            muduleToggleArr[i]=EditorGUILayout.Toggle(ZM.GC.GeneratorConfig.mModuleNameArr[i],muduleToggleArr[i]);
    ...
    {
        string nameSpace = ZM.GC.GeneratorConfig.mMudileNameSpaceArr[index];
 
        for (int i = 0; i < ZM.GC.GeneratorConfig.mMudileNameSpaceArr.Length; i++)
        {
            if (mScriptTxt.Contains(ZM.GC.GeneratorConfig.mMudileNameSpaceArr[i]))
            {
                mScriptTxt= mScriptTxt.Replace(ZM.GC.GeneratorConfig.mMudileNameSpaceArr[i],nameSpace);
            }
        }
        string folder = "";
        if (mFileName.Contains("Data"))
            folder = ZM.GC.GeneratorConfig.folderArr[0];
        else if (mFileName.Contains("Msg"))
            folder = ZM.GC.GeneratorConfig.folderArr[1];
        else if (mFileName.Contains("Logic"))
            folder = ZM.GC.GeneratorConfig.folderArr[2];
        mFilePath = ZM.GC.GeneratorConfig.FramworkPath + ZM.GC.GeneratorConfig.mModuleNameArr[index] + folder + mFileName;
    }
    public void CreateScripts()
    {

        string folderPath= Path.GetDirectoryName(mFilePath);
